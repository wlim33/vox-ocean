...
bool write_png(const std::string& path, const RgbImage& img, int scale) {
    if (img.w <= 0 || img.h <= 0 ||
        img.rgb.size() != (size_t)img.w * img.h * 3) return false;
    if (scale < 1) scale = 1;
    if (scale == 1)
    ...
    return stbi_write_png(path.c_str(), W, H, 3, up.data(), W * 3) != 0;
}
